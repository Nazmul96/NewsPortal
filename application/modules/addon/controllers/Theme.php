  ...
  private $theme_location = APPPATH.'views/themes/';
	...
	function __construct() {
        ...
        if (! $this->session->userdata('isLogIn'))
            redirect('login');
      	$this->load->model(array('themes_model', 'addons_model','backend/view_setting_model'));
    ...
    public function active_theme($name)
    {
        ...
        $this->setu_theme($name);
        $this->session->set_flashdata('message','Theme Active Successfully');
        redirect('addon/theme');
    }


    public function setu_theme($theme_name = '') {

        $this->permission->check_label('theme_setup')->update()->redirect();
    ...
    if ($this->form_validation->run() == TRUE) {
          ...
          if(!empty($response) && ($response->status == 'valid')) {
            ...
            if ($theme_name !== '') {
              ...
              $dir = $this->get_unspace_text($theme_name);
              $target_dir = 'application/views/themes/';
            }
    ...
    public function get_unspace_text($title){
        return $string1 = str_replace(' ', '-', $title); 
    }
