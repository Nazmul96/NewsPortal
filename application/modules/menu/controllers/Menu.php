...
class Menu extends MX_Controller {
    ...
    public function index() {
        ...
        $this->permission->check_label('menu_setup')->read()->redirect();

        $data['menu'] = $this->db->select('*')->from('menu')->get()->result();

        $data['title'] = display('menu');
        $data['module'] = "menu"; 
        $data['page']   = "menu/view_menu_list"; 
    ...
    public function udate_status(){
         $this->permission->check_label('menu_setup')->update()->redirect();
    ...
    public function setup_mobile_menu(){
         $this->permission->check_label('menu_setup')->update()->redirect();

        $id = $this->uri->segment(4);
        $status = ($this->uri->segment(5) == 1) ? 0 : 1;
        $this->db->set('mobile_status',$status)->where('menu_id', $id)->update('menu');
        $this->session->set_flashdata('message', display('update_message'));
        redirect("menu/menu");
    }
    ...
    public function save_menu() {
         ...
         $this->permission->check_label('menu_setup')->create()->redirect();
    ...
    public function setup_menu_content($menu_id=NULL){
        ...
        $data['pages'] = $this->page_model->all_page();
        ...
        $data['title'] = display('menu');
        $data['module'] = "menu"; 
        $data['page']   = "menu/view_setup_menu_content"; 
        echo Modules::run('template/layout', $data);   
...
public function save_content_menu(){
    ...
    if($content_id[0]==NULL){
        ...
        redirect("menu/menu/setup_menu_content/".$menu_id);
    }
}
    ...
    public function sitemap_xml(){
        ...
        $news = $this->db->select('encode_title')->limit('120')->order_by('news_id','DESC')->get('news_mst')->result();
        ...
        foreach ($info as $key => $row) {
            ...
            $xml.="
                <url>
                    <loc>" . base_url('category/').html_escape(@$row->slug) . "</loc>
                    <changefreq> always </changefreq>
                    <priority>1.0</priority>    
                </url>\n";
            }   
        ...
        foreach ($news as $key => $value) {
            ...
            $xml.="
                <url>
                    <loc>" . base_url().html_escape(@$value->encode_title) . "</loc>
                    <changefreq> always </changefreq>
                    <priority>1.0</priority>    
                </url>\n";
            } 
